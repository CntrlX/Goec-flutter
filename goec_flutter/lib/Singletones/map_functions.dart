...
class MapFunctions {
  ...
  Rx<CameraPosition> initialPosition = CameraPosition(
          target: appData.userModel.value.username.contains('+977')
              ? LatLng(28.3949, 84.1240)
              : LatLng(10.6732, 76.6413),
          zoom: 7.5,
          bearing: 0)
      .obs;
  ...
  //             _controller.getChargingStatus(res.bookingId.toString());
  //           } else {
  ...
  addCarMarker(Position event) {
    ...
    markers.add(Marker(
        ...
        icon: BytesMapBitmap(
          MapFunctions().carMarker!,
          height: 35,
          // width: 35,
          // bitmapScaling: MapBitmapScaling.none,
        ),
        position: LatLng(event.latitude, event.longitude),
        rotation: (heading + (math.pi / 180) * -1),
        anchor: Offset(.5, .5)));
  }
  ...
  addMarkerHomePage({
    ...
    markers_homepage.add(Marker(
        ...
        // icon: BitmapDescriptor.fromBytes(status == 'Online'
        //     ? bytesBlue!
        //     : status == 'Busy'
        //         ? bytesGreen!
        //         : bytesGray!),
